...
${JSON.stringify(codeAnalysis, null, 2)}`;
  ...
  log(`  30% Technical execution + Arkiv usage: ${scores.technical ?? "?"}/100`);
  log(`  25% Product viability: ${scores.viability ?? "?"}/100`);
  log(`  25% Scalability: ${scores.scalability ?? "?"}/100`);
  log(`  20% Demo polish + business clarity: ${scores.demo ?? "?"}/100`);
