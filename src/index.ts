...
export async function runPipeline(repoUrl: string, onEvent?: OnEvent) {
  ...
  emit({ type: "agent-done", agentId: "agent1", entityId: agent1Key, payload: agent1Payload, message: "README analysis complete" });
  ...
  emit({ type: "agent-done", agentId: "agent2", entityId: agent2Key, payload: agent2Payload, message: "Code analysis complete" });
  ...
  emit({ type: "agent-done", agentId: "agent3", entityId: agent3Key, payload: agent3Payload, message: "Arkiv evaluation complete" });
  ...
  emit({ type: "agent-done", agentId: "agent4", entityId: agent4Key, payload: agent4Payload, message: "Final report generated" });

  emit({ type: "pipeline-done", report: agent4Payload, entityId: agent4Key, sessionId, message: "Pipeline complete!" });

  return { sessionId, reportEntityId: agent4Key, report: agent4Payload };
}
